...
from sklearn.datasets import make_blobs  # Import make_blobs for synthetic data
from sklearn.svm import SVC  # Import SVC for support vector classification

# 1. Create two linearly-separable blobs
X, y = make_blobs(n_samples=40, centers=2, random_state=42, cluster_std=1.5)  # Generate 2D blobs
y = (y*2 - 1)          # convert {0,1} → {-1,1}

# 2. Hard-margin SVM (very large C)
clf = SVC(kernel="linear", C=1e5)  # Linear SVM with large C for hard margin
clf.fit(X, y)  # Fit SVM to data

# 3. Parameters: w·x + b = 0
w, b = clf.coef_[0], clf.intercept_[0]  # Extract weights and bias
margin = 1/np.linalg.norm(w)  # Compute margin
